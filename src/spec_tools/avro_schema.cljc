...
(defmethod accept-spec 'clojure.spec.alpha/keys [_ spec children _]
  (let [[_ & {:keys [req req-un opt opt-un]}] (impl/extract-form spec)
    ...
    {:type "record"
     :fields (mapv (fn [spec-name spec-type]
                     (cond
                       (:name spec-type) spec-type
                       true (assoc spec-type :name spec-name)))
                   required-un children)}))
...
(defmethod accept-spec 'clojure.spec.alpha/nilable [_ _ children _]
  [{:type "null"} (impl/unwrap children)])
...
(defmethod accept-spec ::visitor/spec [_ spec children _]
  (let [[_ data] (impl/extract-form spec)
        avro-schema-meta (reduce-kv
                           (fn [acc k v]
                             (if (= "avro-schema" (namespace k))
        ...
        extra-info (-> data
                       ...
                       (set/rename-keys {:description :doc}))]
    (merge (impl/unwrap children) extra-info avro-schema-meta)))
